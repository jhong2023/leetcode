...
public class SuggestTree {
    ...
    /**
     * A suggestion string and its score.
     */
    public static final class Suggestion {
        ...
        private Suggestion(String string, int score, int priority) {
            this.string = string;
            this.score = score;
    ...
    /**
     * A rank-ordered list of autocomplete suggestions (from highest to lowest
     * score).
     */
    public static final class List {
        ...
        private List() {
            list = new Suggestion[0];
        }
        
        private List(Suggestion s, int start) {
            list = new Suggestion[] {s};
            suggestion = s;
        ...
        private List(List node, int start) {
            list = node.list;
    ...
    private void collectSuggestions(Suggestion[] a, int[] i, List node) {
        if(node != null) {
    ...
    private void collectData(int[] info, int depth, List node) {
        if(node != null) {
            if(depth > info[0])
                info[0] = depth;
            info[1]++;
            info[2] += node.list.length;
     ...
     * string
     */
    public List getBestSuggestions(String prefix) {
        if(prefix.isEmpty())
            throw new IllegalArgumentException();
    ...
    private void modifyScore(String suggestion, int amount, int i, List node) {
        if(node == null) {
            present = null;
            listPos = -1;
    ...
    private void moveBackwardInList(List node) {
        Suggestion s = present;
        int score = s.score;
        Suggestion[] list = node.list;
        int len = list.length;
        int i = listPos;
        while(i < len && s != list[i])
            i++;
    ...
    private void setLastListElement(Suggestion[] list, List node) {
        if(node == null)
            return;
        int last = list.length - 1;
        int pos = -1;
        Suggestion[] nList = node.list;
    ...
    private List put(String suggestion, int score, int i, List node) {
        if(node == null) {
            present = new Suggestion(suggestion, score, random.nextInt());
            node = new List(present, i);
            putCase = listPos = 0;
        }else if(suggestion.charAt(i) < node.first) {
            ...
            if(listPos != -1) {
                switch(putCase) {
                    ...
                    case 1: moveForwardInList(node.list); break;
    ...
    private List rotateRight(List node) {
        List left = node.left;
    ...
    private List rotateLeft(List node) {
    ...
    private void split(List node, int i) {
        node.down = new List(node, i);
        node.end = (short) i;
        Suggestion[] list = node.list;
        if(list.length == k)
            node.list = Arrays.copyOf(list, k);
    }
    
    private void addToList(List node) {
        Suggestion s = present;
        int score = s.score;
        Suggestion[] list = node.list;
        int i = list.length;
    ...
    private List remove(String suggestion, int i, List node) {
        if(node == null)
            present = null;
        else if(suggestion.charAt(i) < node.first)
    ...
    private List mergeWithChild(List node) {
        List d = node.down;
        d.left = node.left;
        d.right = node.right;
        d.first = node.first;
    ...
    private List siftDown(List node) {
        List left = node.left;
        List right = node.right;
        List cmp;
        if(left != null && right != null)
    ...
    private void removeFromList(List node) {
        Suggestion s = present;
        Suggestion[] list = node.list;
        int len = list.length;
        int i = listPos;
        ...
        if(i == len)
            ...
            if(len < k) {
                ...
                setLastListElement(list, node.down);
                if(list[len - 1] == null)
                    node.list = Arrays.copyOf(list, len - 1);
            }
        }
    }
}